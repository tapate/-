...
public class VoiceMessageExecutor implements IWXMessageExecutor {
    
    private Logger logger = LoggerFactory.getLogger(this.getClass().getName());
    
    @Override
    public BaseMessageResponse executor(BaseMessageRequest baseMessageRequest) {
        System.out.println("[获取语音消息请求事件：]" + baseMessageRequest);
        // 获取语音消息请求事件类
        VoiceMessageRequest voiceMessageRequest = (VoiceMessageRequest)baseMessageRequest;
        Long msgId = voiceMessageRequest.getMsgId();
        String msgType = voiceMessageRequest.getMsgType();
        String fromUserName = voiceMessageRequest.getFromUserName();
        String toUserName = voiceMessageRequest.getToUserName();
        Long createTime = voiceMessageRequest.getCreateTime();
