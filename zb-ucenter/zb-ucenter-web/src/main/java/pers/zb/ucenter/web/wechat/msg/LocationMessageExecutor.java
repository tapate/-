...
public class LocationMessageExecutor implements IWXMessageExecutor {
    
    private Logger logger = LoggerFactory.getLogger(this.getClass().getName());
    
    @Override
    public BaseMessageResponse executor(BaseMessageRequest baseMessageRequest) {
        System.out.println("[获取定位消息请求事件：]" + baseMessageRequest);
        // 获取定位消息请求事件类
        LocationMessageRequest locationMessageRequest = (LocationMessageRequest)baseMessageRequest;
        Long msgId = locationMessageRequest.getMsgId();
        String msgType = locationMessageRequest.getMsgType();
        String fromUserName = locationMessageRequest.getFromUserName();
        String toUserName = locationMessageRequest.getToUserName();
        Long createTime = locationMessageRequest.getCreateTime();
