...
public class VideoMessageExecutor implements IWXMessageExecutor {
    
    private Logger logger = LoggerFactory.getLogger(this.getClass().getName());
    
    @Override
    public BaseMessageResponse executor(BaseMessageRequest baseMessageRequest) {
        System.out.println("[获取视频消息请求事件：]" + baseMessageRequest);
        // 获取视频消息请求事件类
        VideoMessageRequest videoMessageRequest = (VideoMessageRequest)baseMessageRequest;
        Long msgId = videoMessageRequest.getMsgId();
        String msgType = videoMessageRequest.getMsgType();
        String fromUserName = videoMessageRequest.getFromUserName();
        String toUserName = videoMessageRequest.getToUserName();
        Long createTime = videoMessageRequest.getCreateTime();
