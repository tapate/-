package pers.zb.wechat.rpc.api.wxapi.coupon;
...
import pers.zb.wechat.rpc.api.wxapi.coupon.manage.JwCardManageAPI;
import pers.zb.wechat.rpc.api.wxapi.coupon.manage.model.BatchGetCardRtnInfo;
import pers.zb.wechat.rpc.api.wxapi.coupon.manage.model.CardUpdate;
import pers.zb.wechat.rpc.api.wxapi.coupon.manage.model.CommCardRtnInfo;
import pers.zb.wechat.rpc.api.wxapi.coupon.manage.model.DelRtnInfo;
import pers.zb.wechat.rpc.api.wxapi.coupon.manage.model.GetCardDetailRtnInfo;
import pers.zb.wechat.rpc.api.wxapi.coupon.manage.model.GetCardRtnInfo;
import pers.zb.wechat.rpc.api.wxapi.coupon.qrcode.JwQrcodeAPI;
...
public class JwCardManageAPITest {
        ...
        try {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (Integer.parseInt(rtnInfo.getErrcode()) == 0) {
                    ...
                    fail++;
                    System.out.println(
                            "JwCardManageAPI.doBatchGetCard[" + i + "]" + ":失败" + "[" + rtnInfo.getErrmsg() + "]");
                }
                Thread.sleep(1000);
            }
        ...
        try {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (Integer.parseInt(rtnInfo.getErrcode()) == 0) {
                    ...
                    fail++;
                    System.out
                            .println("JwCardManageAPI.doDelCard[" + i + "]" + ":失败" + "[" + rtnInfo.getErrmsg() + "]");
                }
                Thread.sleep(1000);
            }
        ...
        try {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (Integer.parseInt(rtnInfo.getErrcode()) == 0) {
                    ...
                    fail++;
                    System.out.println(
                            "JwCardManageAPI.doGetCardDetail[" + i + "]" + ":失败" + "[" + rtnInfo.getErrmsg() + "]");
                }
                Thread.sleep(1000);
            }
        ...
        try {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (Integer.parseInt(rtnInfo.getErrcode()) == 0) {
                    ...
                    fail++;
                    System.out
                            .println("JwCardManageAPI.doGetCard[" + i + "]" + ":失败" + "[" + rtnInfo.getErrmsg() + "]");
                }
                Thread.sleep(1000);
            }
    ...
    public static boolean doModifystockCardTest(String newAccessToken) {
        ...
        try {
            ...
            for (int i = 0; i < len; i++) {
                onecase = testCases.get(i);
                rtnInfo = JwCardManageAPI.doModifystockCard(newAccessToken, onecase.get("card_id"),
                        Integer.parseInt(onecase.get("increase_stock_value")),
                ...
                if (Integer.parseInt(rtnInfo.getErrcode()) == 0) {
                    ...
                    fail++;
                    System.out.println(
                            "JwCardManageAPI.doModifystockCard[" + i + "]" + ":失败" + "[" + rtnInfo.getErrmsg() + "]");
                }
                Thread.sleep(1000);
            }
    ...
    public static boolean doUnavailableCodeTest(String newAccessToken) {
        ...
        try {
            ...
            for (int i = 0; i < len; i++) {
                onecase = testCases.get(i);
                rtnInfo = JwCardManageAPI.doUnavailableCode(newAccessToken, onecase.get("code"),
                        onecase.get("card_id"));
                if (Integer.parseInt(rtnInfo.getErrcode()) == 0) {
                    pass++;
                    ...
                    fail++;
                    System.out.println(
                            "JwCardManageAPI.doUnavailableCode[" + i + "]" + ":失败" + "[" + rtnInfo.getErrmsg() + "]");
                }
                Thread.sleep(1000);
            }
        ...
        try {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (Integer.parseInt(rtnInfo.getErrcode()) == 0) {
                    ...
                    fail++;
                    System.out.println(
                            "JwCardManageAPI.doUpdateCard[" + i + "]" + ":失败" + "[" + rtnInfo.getErrmsg() + "]");
                }
                Thread.sleep(1000);
            }
    ...
    public static boolean doUpdateCodeTest(String newAccessToken) {
        ...
        try {
            ...
            for (int i = 0; i < len; i++) {
                onecase = testCases.get(i);
                rtnInfo = JwCardManageAPI.doUpdateCode(newAccessToken, onecase.get("code").toString(),
                        onecase.get("card_id").toString(), onecase.get("new_code").toString());
                if (Integer.parseInt(rtnInfo.getErrcode()) == 0) {
                    pass++;
                    ...
                    fail++;
                    System.out.println(
                            "JwCardManageAPI.doUpdateCode[" + i + "]" + ":失败" + "[" + rtnInfo.getErrmsg() + "]");
                }
                Thread.sleep(1000);
            }
